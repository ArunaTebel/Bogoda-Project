...
//       }
        ...
        if (amountField.getText().equals("") || installmentsField.getText().equals("")) {
          ...
          //      int i;
               ...
               // double monthlyPay = instalment_for_the_month(amount, rate, installments, 1);
                ...
                monthlyPay = instalment_for_the_month_by_date(amount, rate, installments, 1, Integer.parseInt(dayfield.getText()), dt.return_index(monthfield.getText()));

                int i;
                int monthNum = Integer.parseInt(month);
                int newMonth;

                dbm.insert("INSERT INTO prcr_loans(emp_id,loan_id,type,amount,installments,rate,month_year,issue_date,date,monthly_amount) VALUES('" + empId + "','" + 0 + "','Current month','" + amount + "','" + installments + "','" + rate + "','" + month_year + "','" + loanDate + "','" + date + "','" + monthlyPay + "')");
                int transaction = dbm.readLastRow("prcr_loans", "tr_no");
                dbm.updateDatabase("prcr_loans", "tr_no", transaction, "loan_id", transaction);
                for (i = 1; i < installments; i++) {

                    monthlyPay = instalment_for_the_month_by_date(amount, rate, installments, i + 1, Integer.parseInt(dayfield.getText()), dt.return_index(monthfield.getText()));
                    month_year = datehandler.forwad_months(year, month, i);
                    date = datehandler.forwad_months(year, month, i)+"-01";

                    dbm.insert("INSERT INTO prcr_loans(emp_id,loan_id,type,amount,installments,rate,month_year,issue_date,date,monthly_amount) VALUES('" + empId + "','" + transaction + "','Previous','" + amount + "','" + installments + "','" + rate + "','" + month_year + "','" + loanDate + "','" + date + "','" + monthlyPay + "')");
                }
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        ...
        if(reply==JOptionPane.YES_OPTION){
            dbm.CheckNDeleteFromDataBase("prcr_loans","loan_id",Integer.parseInt(edit.getText()));
            supplier_id.setSelectedIndex(0);
        ...
        if(evt.getKeyCode() == KeyEvent.VK_ENTER){
            try{
                try {
                    ...
                    supplier_id.setSelectedItem(dbm.checknReturnData("prcr_loans","tr_no",tr_no,"emp_id"));
                    amountField.setText(dbm.checknReturnData("prcr_loans", "tr_no", tr_no, "amount"));
                    installmentsField.setText(dbm.checknReturnData("prcr_loans", "tr_no", tr_no, "installments"));
                    rateField.setText(dbm.checknReturnData("prcr_loans", "tr_no", tr_no, "rate"));

                    String date= dbm.checknReturnData("prcr_loans","tr_no",tr_no,"issue_date");
