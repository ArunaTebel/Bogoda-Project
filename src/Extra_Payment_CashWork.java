...
public class Extra_Payment_CashWork {
    ...
    public void addtoDatabase() {
        ...
        System.out.println(dbm.checknReturnData("pr_workdata_" + st, "code", emp_code, "extra_pay"));
        if (dbm.checknReturnData("pr_workdata_" + st, "code", emp_code, "extra_pay") == null) {
            tot_extrapay = amount;
        } else {
            tot_extrapay = Double.parseDouble(dbm.checknReturnData("pr_workdata_" + st, "code", emp_code, "extra_pay")) + amount;

        }
        System.out.println(tot_extrapay);
        try {

            // dbm.insert("UPDATE " + table_name + " SET " + table_column_need + " ='" + update_element + "' WHERE " + table_column_giving + "='" + row_element + "'");
            dbm.insert("UPDATE  pr_workdata_" + st + "   SET   extra_pay = " + tot_extrapay + " WHERE code= " + emp_code + " ");
